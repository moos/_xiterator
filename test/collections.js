// original underscore collection test suite adapted for _xiterator  
...
(window.addEventListener || window.attachEvent)(window.attachEvent?'onload':'load',function(){
  ...
  test("collections: each", function() {
	  ...
	  _.each([1, 2, 3], "equals(__value, __key + 1, this.msg) ", {msg: 'each iterators provide value and iteration count'});

    answer = null;
    _.each([1, 2, 3],'(answer = __value) &&  == 2 && _.breakLoop()');
    		//function(num){ if ((answer = num) == 2) _.breakLoop(); });
    equals(answer, 2, 'the loop broke in the middle');

    answers = [];
    _.each([1, 2, 3], 'answers.push(__value * this.multiplier)', {multiplier : 5});
    		//function(num){ answers.push(num * this.multiplier);}, {multiplier : 5});
    equals(answers.join(', '), '5, 10, 15', 'context object property accessed');

    answers = [];
    _.forEach([1, 2, 3],'answers.push(__value)');
    		//function(num){ answers.push(num); });
    equals(answers.join(', '), '1, 2, 3', 'aliased as "forEach"');

    answers = [];
    var obj = {one : 1, two : 2, three : 3};
    obj.constructor.prototype.four = 4;
    _.each(obj, 'answers.push(__key);'); 
    		//function(value, key){ answers.push(key); });
    equals(answers.join(", "), 'one, two, three', 'iterating over objects works, and ignores the object prototype.');
    delete obj.constructor.prototype.four;

    answer = null;
    _.each([1, 2, 3], 'answer = _.include(__list, __value) ');
    		//function(num, index, arr){ if (_.include(arr, num)) answer = true; });
    ...
    answers = [];
    _.each({range : 1, speed : 2, length : 3}, 'answers.push(__value)');
    		//function(v){ answers.push(v); });
    ok(answers.join(', '), '1, 2, 3', 'can iterate over objects with numeric length properties');
  });

  test('collections: map', function() {
    var doubled = _.map([1, 2, 3], ' __value * 2'); 
    		//function(num){ return num * 2; });
    equals(doubled.join(', '), '2, 4, 6', 'doubled numbers');

    var tripled = _.map([1, 2, 3], '__value * this.multiplier',  
    		/*function(num){ return num * this.multiplier; }, */{multiplier : 3});
    equals(tripled.join(', '), '3, 6, 9', 'tripled numbers with context');

    var doubled = _([1, 2, 3]).map('__value * 2');
    		//function(num){ return num * 2; });
    equals(doubled.join(', '), '2, 4, 6', 'OO-style doubled numbers');

    var ids = _.map(document.body.childNodes,'__value.id'); 
  ...
  test('collections: all', function() {
    ...
    ok(_.all([0, 10, 28],  '__value %= 2, == 0'),
    		//function(num){ return num % 2 == 0; }), 
    		'even numbers');
    ok(!_.all([0, 11, 28],  'isEven'), 
    		//function(num){ return num % 2 == 0; }), 
    		'an odd number');
  ...
  test('collections: any', function() {
    ...
    ok(!_.any([1, 11, 29],   '__value %= 2, == 0'), 
    		//function(num){ return num % 2 == 0; }), 
    		'all odd numbers');
    ok(_.any([1, 10, 29],  'isEven'),  
    		//function(num){ return num % 2 == 0; }), 
    		'an even number');
  ...
  test('collections: sortBy', function() {
    ...
    people = _.sortBy(people, '__value.age'); 
